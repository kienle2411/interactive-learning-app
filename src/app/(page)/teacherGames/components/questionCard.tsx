...
interface QuestionProps {
    ...
    answers?: AnswerProps[];
    onUpdate?: (updatedQuestion: { question: string; answers: AnswerProps[] }) => void;
    onUpdateChoice?: (choiceId: string, content: string, isCorrect: boolean) => void;
    onDelete?: () => void;
}
...
const QuestionCard: React.FC<QuestionProps> = ({
    ...
    const handleSave = () => {
        const updatedAnswers = tempAnswers.map((answer, index) => {
            const isCorrect = index === tempCorrectAnswerIndex;
            // Call onUpdateChoice for each answer if it has changed
            if (answer.id && (answer.text !== answers[index]?.text || isCorrect !== answers[index]?.isCorrect)) {
                onUpdateChoice?.(answer.id, answer.text, isCorrect);
            }
            return {
                ...answer,
                isCorrect
